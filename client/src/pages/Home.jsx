          ...
          <div className="modal-overlay" onClick={() => setShowBaselineConfigModal(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 520 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem 1rem', marginBottom: '1rem' }}>
                ...
                <label><span className="muted">Day vol ≥ </span><input type="number" step="0.1" min="0" value={baselineConfig.dayVolMult} onChange={(e) => setConfigValue('dayVolMult', parseFloat(e.target.value) || 0)} style={{ width: 56, marginLeft: 4 }} />× prev by {baselineConfig.maxEntryTime || '12:30'}</label>
                ...
                <label style={{ gridColumn: '1 / -1' }}><span className="muted">Move window (3m bars)</span><input type="number" step="1" min="5" max="200" value={baselineConfig.moveWindowBars ?? 11} onChange={(e) => setConfigValue('moveWindowBars', Math.max(5, parseInt(e.target.value, 10) || 11))} style={{ width: 56, marginLeft: 4 }} /><span className="muted" style={{ marginLeft: 8, fontSize: '0.85rem' }}>Segment for 4% move; first entry bar index (20≈60m, 11≈~09:48)</span></label>

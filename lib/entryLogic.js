...
/**
 * Linear regression of (index, high) over candles slice. Returns slope and value at atIndex.
 * Used for downtrend resistance = line through highs evaluated at current bar.
 */
function linearRegressionHigh(candles, startIdx, endIdx, atIndex) {
  let n = 0;
  let sumX = 0, sumY = 0, sumXX = 0, sumXY = 0;
  for (let j = startIdx; j < endIdx && j < candles.length; j++) {
    const x = j;
  ...
  if (n < 2) return { slope: 0, valueAt: sumY / n || 0 };
  const denom = n * sumXX - sumX * sumX;
  const slope = Math.abs(denom) < 1e-10 ? 0 : (n * sumXY - sumX * sumY) / denom;
  const intercept = (sumY - slope * sumX) / n;
  const valueAt = slope * atIndex + intercept;
  return { slope, valueAt };
}

/**
 * Momentum breakout detector (no EMA): 4% sharp move, structure = last N bars,
 * resistance = horizontal or regression of highs (downtrend), enter on volumetric
 * breakout above resistance with strong candle (minimal wicks) and no oversupply at entry.
 */
function findMomentumBreakouts(byDate, sortedDates, options = {}) {
  const skip = (reason) => { if (typeof options.onSkip === 'function') options.onSkip(reason); };
  ...
  const maxEntryTime = options.maxEntryTime ?? '12:15';
  const maxGapUpPct = options.maxGapUpPct ?? null;
  const maxEntryCandleRangePct = options.maxEntryCandleRangePct ?? 1.5;
  const structureBars = options.structureBars ?? 7;
  const maxConsolidationRangePct = options.maxConsolidationRangePct ?? 2;
  const minBreakoutVolumeRatio = options.minBreakoutVolumeRatio ?? 2;
  const stopBelowStructurePct = options.stopBelowStructurePct ?? 0.2;
  const maxUpperWickPctOfBody = options.maxUpperWickPctOfBody ?? 0.3;
  const maxLowerWickPctOfRange = options.maxLowerWickPctOfRange ?? 0.35;
  const maxSupplyBarsAtResistance = options.maxSupplyBarsAtResistance ?? 1;
  const maxPullbackPct = options.maxPullbackPct ?? 5;

  const SHARP_MOVE_WINDOW_BARS = 60;
  const results = [];

  for (let di = 0; di < sortedDates.length; di++) {
    const date = sortedDates[di];
    const candles = byDate[date];
    const minBars = Math.max(structureBars + 3, 21);
    ...
    const dayOpen = candles[0].open;
    if (dayOpen <= 0) { skip('invalid_day_open'); continue; }
    ...
    let iSharp = -1;
    let highSoFar = dayOpen;
    for (let i = 0; i < Math.min(SHARP_MOVE_WINDOW_BARS, candles.length); i++) {
      highSoFar = Math.max(highSoFar, candles[i].high);
      if ((highSoFar - dayOpen) / dayOpen >= sharpMovePct / 100) {
        iSharp = i;
        break;
      }
    }
    ...
    const loopStart = Math.max(structureBars, 20, iSharp);
    for (let i = loopStart; i < candles.length; i++) {
      const c = candles[i];
      const structureStart = i - structureBars;
      const structureWindow = candles.slice(structureStart, i);
      const highs = structureWindow.map((b) => b.high);
      const lows = structureWindow.map((b) => b.low);
      const maxHigh = Math.max(...highs);
      const minLow = Math.min(...lows);
      const mid = (maxHigh + minLow) / 2;
      const structureLow = minLow;
      const rangePct = mid > 0 ? ((maxHigh - minLow) / mid) * 100 : 0;

      let resistance;
      if (rangePct < maxConsolidationRangePct) {
        resistance = maxHigh;
      } else {
        const { slope, valueAt } = linearRegressionHigh(candles, structureStart, i, i);
        resistance = slope < 0 ? valueAt : maxHigh;
      }

      if (c.close <= resistance || c.close <= c.open) continue;

      const t = (c.time || '').slice(0, 8);

      if (maxEntryTime != null) {
        const [mh, mm] = maxEntryTime.split(':').map(Number);
        const [ch, cm] = (c.time || '').split(':').map(Number);
        if (ch * 60 + (cm || 0) >= mh * 60 + mm) { skip(`late_entry_${t}`); continue; }
      }

      const last3 = candles.slice(Math.max(0, i - 3), i);
      const maxLast3High = last3.length > 0 ? Math.max(...last3.map((b) => b.high)) : -Infinity;
      if (c.close <= maxLast3High) {
        skip(`not_above_last3_highs@${t}`);
        continue;
      }

      const avgVol = structureWindow.reduce((s, b) => s + (b.volume ?? 0), 0) / structureBars;
      if (avgVol > 0 && (c.volume ?? 0) < minBreakoutVolumeRatio * avgVol) {
        skip(`volume_ratio@${t}`);
        continue;
      }
      ...
      const resistanceTolerance = resistance * 0.002;
      let supplyBarCount = 0;
      for (const b of structureWindow) {
        if (b.high < resistance - resistanceTolerance) continue;
        const body = Math.abs(b.close - b.open);
        const rng = b.high - b.low;
        const upperWick = b.high - Math.max(b.open, b.close);
        const rejection = (body > 0 && upperWick >= 0.3 * body) || (rng > 0 && upperWick >= 0.3 * rng);
        if (rejection) supplyBarCount++;
      }
      if (supplyBarCount > maxSupplyBarsAtResistance) {
        skip(`oversupply_${supplyBarCount}_at_resistance@${t}`);
        continue;
      }
      if (maxHigh > resistance * 1.005) {
        skip(`overhead_supply@${t}`);
        continue;
      }

      const barRange = c.high - c.low;
      const body = c.close - c.open;
      if (barRange > 0 && body / barRange < 0.4) {
        skip(`doji_${(body / barRange * 100).toFixed(0)}pct@${t}`);
        continue;
      }
      const upperWick = c.high - c.close;
      if (body > 0 && upperWick > maxUpperWickPctOfBody * body) {
        skip(`upper_wick_${(upperWick / body * 100).toFixed(0)}pct@${t}`);
        continue;
      }
      const lowerWick = Math.min(c.open, c.close) - c.low;
      if (barRange > 0 && lowerWick > maxLowerWickPctOfRange * barRange) {
        skip(`lower_wick_${(lowerWick / barRange * 100).toFixed(0)}pct@${t}`);
        continue;
      }

      if (maxEntryCandleRangePct != null && Number.isFinite(maxEntryCandleRangePct) && c.open > 0) {
        const entryBarRangePct = (barRange / c.open) * 100;
        if (entryBarRangePct > maxEntryCandleRangePct) {
          skip(`entry_bar_range_${entryBarRangePct.toFixed(1)}pct@${t}`);
          continue;
        }
      }

      // Max pullback: from move high (since sharp move) to structure low — skip if pullback too deep
      if (maxPullbackPct != null && Number.isFinite(maxPullbackPct)) {
        const moveHigh = Math.max(...candles.slice(iSharp, i + 1).map((b) => b.high));
        const pullbackLow = Math.min(...candles.slice(iSharp, i + 1).map((b) => b.low));
        const pullbackPct = moveHigh > 0 ? ((moveHigh - pullbackLow) / moveHigh) * 100 : 0;
        if (pullbackPct > maxPullbackPct) {
          skip(`pullback_${pullbackPct.toFixed(1)}pct@${t}`);
          continue;
        }
      }

      let suggestedStop = structureLow * (1 - stopBelowStructurePct / 100);
      if (c.close > 0 && (c.close - suggestedStop) / c.close > maxSlPct / 100) {
        suggestedStop = c.close * (1 - maxSlPct / 100);
      }
      ...
      results.push({
        ...
        suggestedEntry: Math.round(c.close * 100) / 100,
        suggestedStop,
        entryBarVolume: c.volume ?? 0,
        consolidationAvgVolume: Math.round(avgVol),
      });
      break;
    }
  }
...
function findReversalBreakouts(byDate, sortedDates, sharpMovePct = 4, pullbackNearPct = 2, maxGapUpPct = null, maxEntryCandleRangePct = 1.5, maxSlPct = 2, getPrevDayVolume = null, maxPullbackPct = 5, maxConsolidationRangePct = 2, getPrevDayCloseDaily = null, getDayOpenDaily = null, onSkip = null, maxEntryTime = null) {
  ...
  for (let di = 0; di < sortedDates.length; di++) {
    ...
    // Start after sharp move. When prev day bars seeded EMA, we have valid EMA from bar 0 so start at iSharp; else need 20 bars for EMA.
    const loopStart = prevDayCandles.length > 0 ? iSharp : Math.max(20, iSharp);
    for (let i = loopStart; i < candles.length; i++) {
      const c = candles[i];
      const e = ema20[i];
      ...
      // runLen >= 1: allow breakout after one bar of consolidation (e.g. NITINSPIN 10:06 when 10:03 was first near-EMA bar)
      const wouldBreakout = runLen >= 1 && c.close > consHighRecent && c.close > e;
      if (wouldBreakout) {
        const t = (c.time || '').slice(0, 8);
        ...
        // Time filter: skip entries at or after maxEntryTime (e.g. "11:30")
        if (maxEntryTime != null) {
          const [mh, mm] = maxEntryTime.split(':').map(Number);
          const [ch, cm] = (c.time || '').split(':').map(Number);
          if (ch * 60 + (cm || 0) >= mh * 60 + mm) { skip(`late_entry_${t}`); continue; }
        ...
        // Upper wick filter: large upper wick vs body = selling pressure at highs = weak breakout
        const upperWick = c.high - c.close;
        if (body > 0 && upperWick > 0.5 * body) { skip(`upper_wick_${(upperWick / body * 100).toFixed(0)}pct_of_body@${t}`); continue; }
        ...
        let suggestedStop = Math.round((stopFromLow - 0.005 * stopFromLow) * 100) / 100;
        // Cap stop at maxSlPct so we don't skip valid breakouts when last-bars' low is far (e.g. NITINSPIN 10:06)
        if (maxSlPct != null && Number.isFinite(maxSlPct) && c.close > 0 && suggestedStop < c.close) {
          const slPct = ((c.close - suggestedStop) / c.close) * 100;
          if (slPct > maxSlPct) {
            suggestedStop = Math.round(c.close * (1 - maxSlPct / 100) * 100) / 100;
          }
        }
        ...
        // Require close above actual last 3 bars' high (consHighRecent can be stale when price left EMA)
        const last3Bars = candles.slice(Math.max(0, i - 3), i);
        const last3Highs = last3Bars.map((b) => b.high);
        const maxLast3High = last3Highs.length > 0 ? Math.max(...last3Highs) : -Infinity;
        if (c.close <= maxLast3High) {
          skip(`not_above_last3_highs_${maxLast3High.toFixed(2)}_close_${c.close.toFixed(2)}@${t}`);
          continue;
        }
        results.push({
 ...
 * Momentum entries come from findMomentumBreakouts (no EMA).
 */
function runEntryLogic(byDate, sortedDates, options = {}) {
  ...
  const reversalBreakouts = findMomentumBreakouts(byDate, sortedDates, {
    sharpMovePct: options.sharpMovePct ?? 4,
    maxSlPct,
    getPrevDayVolume,
    ...
    stopBelowStructurePct,
    maxUpperWickPctOfBody,
    maxLowerWickPctOfRange,
    maxSupplyBarsAtResistance,
    maxPullbackPct,
  });

...
export const POSITION_VALUE     = 20000;   // ₹ per trade
...
export function addPosition(fields) {
  ...
  const qty = Math.floor((fields.positionValue ?? POSITION_VALUE) / fields.entryPrice);
  const position = {
    id,
...
/**
 * Close every open position using exit prices from priceBySymbol (symbol → LTP).
 * Missing or invalid price falls back to entryPrice. Returns closed position rows.
 */
export function closeAllOpenAtPrices(dateStr, timeStr, priceBySymbol, exitReason) {
  const positions = loadPositions();
  const closed = [];
  let changed = false;
  for (let i = 0; i < positions.length; i++) {
    ...
    const raw = priceBySymbol[sym];
    const exitPrice =
      raw != null && Number.isFinite(raw) && raw > 0 ? raw : positions[i].entryPrice;
    const { entryPrice, qty } = positions[i];
    const pnl = Math.round((exitPrice - entryPrice) * qty * 100) / 100;
    Object.assign(positions[i], {
      status: 'closed',
      exitTime: `${dateStr} ${timeStr}`,
      exitPrice,
      pnl,
      exitReason,
    });
    closed.push(positions[i]);
    changed = true;
  }
  if (changed) savePositions(positions);
  return closed;
